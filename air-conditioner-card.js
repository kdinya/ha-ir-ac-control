/**
 * Air Conditioner Card (Universal IR Remote Edition)
 ...
 *
 * Every entity is optional except the remote (needed only to send IR
 * commands). Everything else is configured through the visual editor:
 *  - "Entities" tab — remote, room temperature/humidity, weather, contact
 *    sensor, helpers.
 *  - "Buttons & IR codes" tab — ON/OFF, second mode (TURBO/ECO/custom),
 *    one code per temperature. Each field has a "Learn" button that waits
...
const I18N = {
  uk: {
    ...
    hint_temp_codes: 'Кнопки "+" та "-" не мають окремих кодів — вони завжди надсилають код температури, яка вийде після натискання, з таблиці нижче.',
  ...
  en: {
    ...
    hint_temp_codes: 'The "+" and "-" buttons have no separate codes — they always send the code for the resulting temperature from the table below.',
  ...
  ru: {
    ...
    hint_temp_codes: 'Кнопки "+" и "-" не имеют отдельных кодов — они всегда отправляют код температуры, которая получится после нажатия, из таблицы ниже.',
...
class AirConditionerCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      // Font uploaded by the user on the "Appearance" tab (data: URI), if any.
      // Falls back to the embedded default seven-segment font.
      custom_font_data: config.custom_font_data || '',

      // Normalize to always carry a unit (pxCqw appends "cqw" to a bare
      // number). Without this, typing e.g. "12" instead of "12cqw" produces
  ...
  _injectGlobalFont() {
    const fontId = 'ha-ir-ac-control-seg7-font';
    const fontSrc = this.config.custom_font_data || `data:font/woff;base64,${DEFAULT_SEG7_FONT_B64}`;
  ...
  _act(act) {
    ...
    if (act === 'power') {
      ...
      // Mode 2 (TURBO/ECO/...) only sends its own IR code — it never touches
      // the displayed/stored temperature.
      if (!this._local.power) {
      ...
      // No separate +/- codes: always send the code for the resulting temperature.
  ...
  _css() { return `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .mode-row { display: flex; align-items: center; gap: 0.7cqw; font-size: 2.5cqw; font-weight: 800; letter-spacing: 0.2cqw; color: #1c2820; opacity: var(--mode-op); transition: opacity 0.3s; }
    .mode-icon { font-size: calc(2.5cqw + 0.2cqw); display: inline-block; animation: bob 3.5s ease-in-out infinite; }
    ...
    .status-ac-icon { width: 2.8cqw; height: 2.8cqw; transition: transform 0.3s; }
    ...
    .weather-val { font-size: 3.2cqw; letter-spacing: 0.02cqw; }
    .weather-unit { font-family: -apple-system, sans-serif; font-size: calc(3.2cqw * 0.6); font-weight: 700; color: #1c2820; opacity: 0.8; margin-left: 0.2cqw; }
    ...
    .wind-val { font-size: 3.2cqw; letter-spacing: 0.02cqw; }
    ...
    .left-group { display: flex; flex: 0 0 auto; justify-content: flex-start; align-items: center; }
    .center-group { display: flex; flex: 0 1 auto; align-items: center; gap: 1cqw; justify-content: center; }
    .control-slot { position: relative; width: min(8.2cqw, ${this.config.controls_height}); height: min(8.2cqw, ${this.config.controls_height}); flex-shrink: 0; }
    ...
    .btn::before {
      ...
      background: radial-gradient(circle at 50% 40%, rgba(0,0,0,0) 55%, rgba(0,0,0,0.35) 100%);
    }
    .btn::after {
...
// Guard against a duplicate customElements.define() if this resource gets
// re-evaluated in the same browser tab (e.g. HA's "Reload resources").
// A real page reload is still required to pick up new class code.
if (!customElements.get('ha-ir-ac-control-card')) {
  customElements.define('ha-ir-ac-control-card', AirConditionerCard);
} else {
  console.warn(
    `AIR-CONDITIONER-CARD: v${CARD_VERSION} loaded, but an older class is already registered ` +
    `in this tab. Close the tab (or hard-reload) to actually run the new version.`
  );
}
...
class AirConditionerCardEditor extends HTMLElement {
  ...
  // so typing/dragging in the editor isn't interrupted mid-gesture.
  set hass(hass) {
    this._hass = hass;
  ...
  async _learn(cmdKey, buttonEl) {
    ...
    // NOTE on why this doesn't scrape a persistent_notification for the code:
    // Broadlink's remote.learn_command stores the learned IR code directly in
    // its own server-side storage (broadlink_<mac>_codes) and never exposes
    // that code back to the frontend — not via the service call's return
    // value, not via a notification message (that notification only ever
    // says "Press the button", both before and after a successful catch).
    // So instead of the code itself, we store the command KEY we asked
    // Broadlink to learn (e.g. "on", "24"). remote.send_command already
    // knows how to resolve a short, non-base64 value like that by name
    // against the remote's own storage (see _sendIR below) — no code needed.
    const learnTimeout = 15;
    try {
      ...
      // The service call only throws on hard failures (missing device,
      // remote unavailable/unauthorized, etc). A plain timeout with no
      // signal received is swallowed by Home Assistant itself and logged
      // server-side only, so we can't distinguish "learned" from "timed
      // out with no press" purely on the frontend. Given that, treat a
      // clean resolve as success — this matches what actually happens for
      // Broadlink in the overwhelming majority of cases (the call resolves
      // fast, well before the 15s timeout, once a signal is caught).
      this._setCommand(cmdKey, String(cmdKey));
  ...
  _render() {
    ...
    if (this._tab === 'commands') {
      ...
      const hintPlusMinus = document.createElement('div');
      hintPlusMinus.className = 'hint';
      hintPlusMinus.textContent = this._t('hint_temp_codes');
      panel.appendChild(hintPlusMinus);
  ...
  // -------------------------------------------------------------------------
  // Seven-segment font: upload a custom file (ttf/otf/woff/woff2), stored as
  // a data: URI directly in the card config. "Reset" removes it so the
  // card falls back to the font built into the resource file.
  // -------------------------------------------------------------------------
  _fontUploadField() {
    const wrap = document.createElement('div');
    wrap.className = 'field';
    const hasCustom = !!this._config.custom_font_data;
    wrap.innerHTML = `
      <label>${this._t('f_font_upload')}</label>
      <div class="hint">${hasCustom ? this._t('font_custom_active') : this._t('font_default_active')}</div>
      <div class="row">
        <input type="file" accept=".ttf,.otf,.woff,.woff2" class="font-file-input" />
        <button type="button" class="learn-btn font-reset-btn" ${hasCustom ? '' : 'disabled'}>${this._t('reset_btn')}</button>
      </div>
    `;
    const fileInput = wrap.querySelector('.font-file-input');
    fileInput.addEventListener('change', () => {
      const file = fileInput.files && fileInput.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => this._setField('custom_font_data', reader.result);
      reader.readAsDataURL(file);
    });
    wrap.querySelector('.font-reset-btn').addEventListener('click', () => this._setField('custom_font_data', ''));
    return wrap;
  }
  ...
  _positionDragHandles(wrap, liveCard) {
    ...
    POSITIONABLE_ELEMENTS.forEach((item) => {
      ...
      this._attachDragBehavior(handle, item.key, cqwPx, wrap, liveCard);
      wrap.appendChild(handle);
    });
  }

  // Mirrors the exact "translate(...) scale(...)" formula _initHTML() bakes
  // into each element's inline style, so we can patch it live without a
  // full re-render (see _attachDragBehavior commit()).
  _transformStringFor(key, nx, ny) {
    const [xCentered, yCentered] = POS_AXIS_CENTERED[key] || [false, false];
    const xTerm = xCentered ? `calc(-50% + ${nx}cqw)` : `${nx}cqw`;
    const yTerm = yCentered ? `calc(-50% + ${ny}cqw)` : `${ny}cqw`;
    const scale = numScale(this._config[`offset_${key}_scale`]);
    return `translate(${xTerm}, ${yTerm}) scale(${scale})`;
  }

  _attachDragBehavior(handle, key, cqwPx, wrap, liveCard) {
    ...
    const commit = (e) => {
      ...
      // The live preview card's own _updateDOM() never touches inline
      // transforms (those are only baked in once, in _initHTML()), so
      // without this the handle would visually snap back to the pre-drag
      // spot the instant the drag's temporary CSS transform is cleared
      // above, then only jump to the correct spot on some later, unrelated
      // re-render. Patch the real element's transform immediately so the
      // handle can be repositioned against its true, up-to-date position.
      if (liveCard && liveCard.shadowRoot) {
        const targetEl = liveCard.shadowRoot.querySelector(`[data-el="${dataEl}"]`);
        if (targetEl) targetEl.style.transform = this._transformStringFor(key, nx, ny);
      }
      this._positionDragHandles(wrap, liveCard);
    };
...
if (!customElements.get('ha-ir-ac-control-card-editor')) {
  ...
  console.warn(`AIR-CONDITIONER-CARD: editor v${CARD_VERSION} loaded, but an older editor class is already registered in this tab. Close the tab (or hard-reload) to load it.`);
}
